...
// Eager (non-lazy) imports so server-side rendering produces full HTML.
// The client app keeps code-splitting for performance; this server-only tree
// renders every page synchronously (renderToString cannot suspend on lazy).
...
/**
 * Server-only App tree: identical routing to the client App, but with eager
 * imports and NO <Suspense>. This is what entry-server.tsx renders so every
 * route produces complete, crawlable HTML (title, canonical, hreflang, JSON-LD
 * and body content all present in the static file).
 */
